...
public class Panel extends JPanel {
	...
	public void paint(Graphics g) {
		...
		if (currentScreen == 4) {
			...
			net.abysmal.clickerconquest.graphics.Graphics.drawButton(G, 4, 0, 0, (GameWindow.width / 5) * 2, (int) (GameWindow.height / 6.75), "1,947,360.852", (int) (GameWindow.height / 6.75) - 70);
		}
		for (int i = 0; i < window.buttons.length; i++) {
			if (window.buttons[i] != null && window.buttons[i].getScreen() == currentScreen) {
				net.abysmal.clickerconquest.graphics.Graphics.drawButton(G, window.buttons[i].getMargin(), window.buttons[i].getX(), window.buttons[i].getY(), window.buttons[i].getWidth(), window.buttons[i].getHeight(), window.buttons[i].getText(), window.buttons[i].getFontSize());
			}
		}
